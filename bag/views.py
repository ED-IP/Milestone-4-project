    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['item_by_size'].keys():
                ...
                messages.success(request, f'Updated size {size.upper()} {product.title} quantity to {bag[item_id]["item_by_size"][size]}')
            else:
                bag[item_id]['item_by_size'][size] = quantity
...
def adjust_bag(request, item_id):
    """ Adjust the quantity of the products to a specific amount """
    ...
    if size:
        if quantity > 0:
            ...
            if not bag[item_id]['item_by_size']:
                bag.pop(item_id)
            messages.success(request, f'Removed size {size.upper()} {product.title} from your bag')
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(request, f'Updated {product.title} quantity to {bag[item_id]}')
            ...
            bag.pop(item_id)
            messages.success(request, f'Remove {product.title} from your bag')
