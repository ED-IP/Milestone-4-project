...
def all_products(request):
    """ A view to show all products, sorting and searchs """
    ...
    if request.GET:
        ...
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
                messages.error(request, "You didn't enter any search term!")
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Failed to add product. Please review that the form has valid data.')
    else:
        form = ProductForm()
